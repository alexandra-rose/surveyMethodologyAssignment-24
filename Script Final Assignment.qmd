...
```{r}
data_cleaning_3 <- data_cleaning_2 %>%
  mutate(
...
```{r}
data_cleaning_4 <- data_cleaning_3 %>%
  mutate(
    gender = case_when(
      d10 == 1 ~ "male",
      d10 == 2 ~ "female"
...
```{r}
merged_df <- merge(data_cleaning_4, Data_Extra_by_Country, by = "country_name", all.x = TRUE)
```
...
df_descriptive$binary_qc19 <- ifelse(df_descriptive$qc19 == 1, 1, ifelse(df_descriptive$qc19 == 2, 0, NA))

# binary_qc19 is our binary variable with 1 for Yes, 0 for No, and NA for Don't know
summary(df_descriptive$binary_qc19)
```

```{r}
df_descriptive <- left_join(df_descriptive, df_country_1 %>% select(country_name, prop_gndr_bin), by = "country_name")

df_descriptive <- left_join(df_descriptive, df_country_3 %>% select(country_name, prop_lgb_yes), by = "country_name")


df_descriptive <- left_join(df_descriptive, df_country_2 %>% select(country_name, prop_dis_wide), by = "country_name")
#qc1
```

# FINAL DATA SET needs to be checked.

```{r}
data_final <- df_descriptive |> 
  select(serialid,country_name,binary_qc19,qc19,Cat_age_four,Cat_age_six,Cat_age_seven,gender, qc20_multinominal,prop_gndr_bin,d1_ordinal, sd3_cat, sd1_7_multinomial,d10,d11, d8,d60_ordinal,sd2t,sd2_5, prop_dis_wide, qc15_1_ordinal, prop_lgb_yes,d25,adp_general)
