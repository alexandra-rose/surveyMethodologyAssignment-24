...
df_country_1 <- 
  ...
  group_by(country_name) |> 
  summarize(
    prop_gndr_bin = (sum(qc20_multinominal == "Yes") / n())*100, # Calculate proportion 
...
# Perform descriptive analysis for Unemployment and GDP per capita
summary(df_descriptive$unemp)
summary(df_descriptive$GDPpc)

# Create histograms for visualizing the distribution of Unemployment and GDP per capita
ggplot(df_descriptive, aes(x = unemp)) +
  geom_histogram(binwidth = 1, fill = "skyblue", color = "black") +
  labs(title = "Distribution of Unemployment", x = "Unemployment Rate")

ggplot(df_descriptive, aes(x = GDPpc)) +
  geom_histogram(binwidth = 1000, fill = "lightgreen", color = "black") +
  labs(title = "Distribution of GDP per Capita", x = "GDP per Capita")

# Cross-tabulate the non-numeric variables with the target variable
cross_table <- table(df_descriptive$qc19_multinominal, df_descriptive$unemp)
print(cross_table)
...
# Cross-tabulate the non-numeric variables with the target variable
cross_table1 <- table(df_descriptive$qc19_multinominal, df_descriptive$GDPpc)
print(cross_table1)

# Perform chi-square test
chi_square_test1 <- chisq.test(cross_table1)
print(chi_square_test1)
```

Chi squared statistic is a measure of the strength of the relationhsip, for both we have very high numbers and both tests are statistically significant. Thus we can reject the null hypothesis that there is non association between the variables. There are skewed or asymmetric distributions for both.
...
Making the binary predictive model using the new merged and reduced data set.
...
#proportion variables to be between 0 and 1
Data$prop_gndr_bin <- Data$prop_gndr_bin / 100
Data$prop_lgb_yes <- Data$prop_lgb_yes / 100
Data$prop_dis_wide <- Data$prop_dis_wide / 100
...
```{r}
test_model <- glm(binary_qc19 ~ male + d11 + I(d11^2) + political_ideology +
...
The variable qc_15_1 shows that being in total disagreement on being labeled as part of a sexual minority, related to a extreamlly high non-supportive actite towards believen that transgender individuals should be able to change their civil documents.

**Country Specific Variables**

General support in a country for accepting that there should be a third gender option and agreeing that transgender discrimination is widespread in their own country indicate a really high supportive general attitude towards allowing transgenderer individuals to change their civil documents. Unemployment resulted significant and positive, suggesting that higher unemployment rates within a country might be associated with greater support for transgender rights, which could indicate a variety of social dynamics.
...
```{r}
ModelS = binary_qc19 ~ male + d11 + I(d11^2)  + political_ideology +                         Religion_cat + sd1_7_factor + d60_ordinal + qc15_1_ordinal
```
