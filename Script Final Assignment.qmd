...
```{r}
# Calculate the counts of "Yes" and "No" responses for each country
binary_qc19_data <- df_descriptive %>%
  group_by(country_name) %>%
  summarise(
    count_yes = sum(binary_qc19 == 1, na.rm = TRUE),  # Count of "Yes" responses
    count_no = sum(binary_qc19 == 0, na.rm = TRUE)   # Count of "No" responses
  ) %>%
  ungroup()
...
Data_final_Cntry <- Country_level |>
   select(count_yes,count_no , prop_gndr_bin, prop_dis_wide,prop_lgb_yes, trans_legal_protection, Gdp_per_capita, Unemployment)
```
...
```{r}
logit_model2 <- glm(binary_qc19 ~ male + d11 + I(d11^2)  + political_ideology +                         Religion_cat + sd1_7_factor + d60_ordinal + qc15_1_ordinal, 
                    family = binomial(link = "logit"), data = data_reg_ind)

summary(logit_model2)
```

### Regression Country Level

Due to our dependent variable now being a proportion we will use another type of regression model that accommodates for this.

```{r}
glm_model <- glm(cbind(count_yes, count_no) ~ ., 
                 family = binomial(link = "logit"), data = Data_final_Cntry)
