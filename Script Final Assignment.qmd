...
## DESCRIPTIVE ANALYSIS - Explaining cross-country differences
...
Upon examining our dataset, we have identified that the variable age_stopped_education is the numeric variable that has the largest number of NA's. Given its significance and the extent of missing information, we have opted to proceed with imputing these missing values.
...
# using method CART for age_stopped_education
...
# using method LASSO for age_stopped_education
...
# using method RANDOMFOREST for age_stopped_education
...
Plot for selecting the best imputation method
...
Data_final_Ind$age_stopped_education <- ifelse(a.na(Data_final_Ind$age_stopped_education), 
...
```{r}
any_na_in_dataset <- any(is.na(Data_final_Ind$age_stopped_education))
...
prediction <- as.factor(ifelse(probability > 0.607, "Yes", "No"))

confusionMatrix(prediction, testing$bin)
```

The above presented confusion matrix drawing on the best possible threshold defined by the ROC curve reports a 76.61% specificity rate and 73.35% sensitivity rate. This means that in 77% and 73% of the times, respectively, a new observation is correctly classified into the 'Yes' or 'No' class. As a whole, this statistical model reports an accuracy of 74.44%.
...
```{r}
ctrl <- trainControl(method = "repeatedcv", number = 10, classProbs = T, summaryFunction=twoClassSummary, verboseIter = T)
```
...
We can observe an improvement in the model's accuracy, now at 74.99%. There's also a notable increase in sensitivity, which is currently at 63.29%, suggesting better detection of positive cases. Specificity remains nearly unchanged and is still reflecting strong predictive power for negative cases. These results are more promising and indicate that the model is becoming more robust in terms of overall predictive accuracy and the ability to identify positive instances in comparison to the previous model.
...
prediction <- as.factor(ifelse(dtProb[,"Yes"] > 0.583, "Yes", "No"))

confusionMatrix(prediction, testing$binary_qc19)

```

The model has shown a slight improvement in Accuracy, Specificity and Sensitivity. However, is still really unbalanced. These changes suggest that while the model is becoming better at correctly identifying negative cases and overall correct predictions, it is less effective at identifying positive cases.
...
prediction <- as.factor(ifelse(rfProb[,"Yes"] > 0.643, "Yes", "No"))

confusionMatrix(prediction, testing$binary_qc19)
```

The model shows a slight decrease in Accuracy. Howewver, the balance for Specificity and Sensitivity has improved.
...
```{r}
prediction <- as.factor(ifelse(gbmProb[,"Yes"] > 0.608, "Yes", "No"))

confusionMatrix(prediction, testing$binary_qc19)

```

This is the best model so far, with an Accuracy of 74.89%, Sensitivity of 74.70% and a Specificity of 75.01%
...
For the d11 plot, it shows a more complex, non-linear relationship. This is something we adressed by adding the quadratic age term.

Both plot further evidence the insight we have gatherer throught this analisis.
...
```{r}
nnPredictedClass <- as.factor(ifelse(nnProb[,"Yes"] > 0.629, "Yes", "No"))
confusionMatrix(nnPredictedClass, testing$binary_qc19)
```

The Accuracy levels decrease to 73%, as well as, the Specificity to 75.24%. For the Sensitivity there is an improvement to 72%.
