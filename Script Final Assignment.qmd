...
na_summary <- colSums(is.na(Data_final_Ind))
...
We deemed that it is unwise to impute data in the target variable, and that in the other cases imputation might cause more harm than benefit. However we considered that for the age_stopped_education variable it could be valuable to impute the data and since it is a numerical variable it may be more accurate.
...
impute_vars <- c("age_stopped_education", "d11", "political_ideology") # Add other variables as needed

# Create a copy of the original dataset with only the specified variables
impute_data <- Data_final_Ind[, impute_vars]

# Perform multiple imputation using polyreg method
num_imputations <- 5  # Adjust as needed
imputed_data <- mice(impute_data, method = "pmm", m = num_imputations)

# Combine the imputed datasets into a single dataset
completed_data <- complete(imputed_data)

# Replace the imputed variables in Data_final_Ind with the completed values
Data_final_Ind[impute_vars] <- completed_data[impute_vars]

Data_final_Ind #USE THIS DATA SET WITH IMPUTED VALUES
```

We check the distribution of the imputation and we see a generally similar distribution, so we think this imputation is promising!

```{r}
# Before imputation
before_imputation <- Data_final_Ind$age_stopped_education

# After imputation
after_imputation <- completed_data$age_stopped_education

# Create histograms for visual comparison
par(mfrow = c(1, 2))  # Set up a 1x2 plotting layout

hist(before_imputation, main = "Before Imputation", xlab = "Age Stopped Education", col = "skyblue")
hist(after_imputation, main = "After Imputation", xlab = "Age Stopped Education", col = "lightgreen")

```

make the binary predictive model using the new merged and reduced dataset, try all the models to see which is best

## Predicting the binary target variable

In order to be able to predict the target variable, we need to define our success that we want to predict. In this case it is support for LGBTQ minorities. We see that the data is balanced in our favor below, there are more Yes' than there are Nos, this is important as it will be easier for us to predict the Yes' than the Nos.

```{r}
#We want to check if there are more Yes' or more Nos to see whicgh would be better to predict
Data_final_Ind |> 
  count(binary_qc19==1) 

## Definition of yes
Data_final_Ind$Yes = factor(Data_final_Ind$binary_qc19==1) 
Data_final_Ind$binary_qc19 = NULL

prop.table(table(Data_final_Ind$Yes))
```

Next we move to split the data into testing and training with a 75% split.

```{r}
set.seed(123)

in_train <- createDataPartition(Data_final_Ind$Yes, p = 0.75, list = FALSE)  # 75% for training

# Extracting the indices from the first column of in_train
indices <- in_train[, 1]

# Subsetting the data frame using the extracted indices
training <- Data_final_Ind[indices, ]
```

```{r}
data_reg_ind <- Data_final_Ind |> #changed variable name here
  select(Yes,male, d11 ,political_ideology,Religion_cat , sd1_7_factor, age_stopped_education, d60_ordinal,sd2_5,area_type,qc15_1_ordinal)
...
logit_model <- glm(Yes ~ ., family=binomial(link='logit'), data= data_reg_ind)
...
```{r}
logit_model2 <- glm(Yes ~ male + d11 + political_ideology +                         Religion_cat + sd1_7_factor + d60_ordinal + qc15_1_ordinal, 
...
```{r}
logit_model3 <- glm(Yes ~ male + d11 + I(d11^2)  + political_ideology +                         Religion_cat + sd1_7_factor + d60_ordinal + qc15_1_ordinal, 
                    family = binomial(link = "logit"), data = data_reg_ind)
...
testing <- Data_final_Ind[-indices,]
nrow(training)
nrow(testing)
```
...
```{r}
logit.model = glm(Yes ~ . , data = training, family = binomial)
...
## Prediction using the LDA model (doesnt work, data format?)

```{r}
# Assume 'my_data' is your dataset with predictors and response variable
# Create the LDA model
lda_. model <- lda(Yes ~ ., data = training)

# Summary of the LDA model
summary(lda_model)

# Make predictions using the LDA model
lda_probability <- predict(lda_model, newdata = testing)$posterior

# Convert the probabilities into a data frame
lda_probability_df <- as.data.frame(lda_probability)

# Display the head of the probabilities
head(lda_probability_df)

```

### Prediction using Benchmark model

```{r}
bench.model = glm(Yes ~ 1, family=binomial(link='logit'), data=training)
...
## Using the correct model to predict support (here the logit model- assumed)

To predict the support of LGBTQ minorities, we apply the Bayes' rule of maximum probability:

```{r}
prediction <- max.col(probability) # 0 means false, 1 means true
head(prediction)
```
...
```{r}
prediction = predict(logit.model, newdata=testing)$binary_qc19
head(prediction)
```
...
```{r, echo=FALSE}
confusionMatrix(prediction, testing$Yes)$table
confusionMatrix(prediction, testing$Yes)$overall[1:2]
```
