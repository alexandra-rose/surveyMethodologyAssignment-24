...
```{r}
merged_df <- merge(data_cleaning_2, Data_Extra_by_Country, by = "country_name", all.x = TRUE)
```
...
Binary in an individual level

```{r}
# Our dependent variable will be trated as binary
...
Individual_data <- df_descriptive %>%
  select(serialid,country_name,isocntry, binary_qc19)
```

Binary in a country level.

```{r}
#frederick i believe created already a variable for prop yes, ask
```

INDIVIDUAL LEVEL(not yet ready)
...
df_descriptive <- df_descriptive %>% 
  #filter(!d1 %in% c(97, 98)) %>%
  rename(political_ideology = d1) 

#removes observations with 'Non-response' and 'Don't know'
...
df_descriptive <- df_descriptive %>%
  mutate(Religion_cat = case_when(
    ...
    sd3 == 11 ~ "Hindu",
    sd3 == 12 ~ "Atheist",
    sd3 == 13 ~ "Non believer or agnostic",
    sd3 == 14 ~ "Other",
    sd3 == 15 ~ "Refusal (SPONTANEOUS)",
    sd3 == 16 ~ "DK",
    TRUE ~ NA_character_ # This will assign NA to any code that doesn't match
  )) 
#%>%
  # Removing rows with NA in sd3_cat if desired
  #filter(!is.na(sd3_cat))
...
#df_descriptive <- df_descriptive %>%
  #mutate(sd1_7_binary = case_when(
    #sd1_7 == 1 ~ 1,    # Yes
    #sd1_7 == 2 ~ 0,    # No
    #TRUE ~ NA_real_  ))   # Refusal and Don't know are treated as missing


Individual_data <- left_join(Individual_data, data %>% select(serialid, sd1_7_factor), by = "serialid")
```
...
Country_level <- left_join(Country_level, df_country_1 %>% select(country_name, prop_qc19_yes),  by = "country_name")
...
Unemployment by country, got the excel just in case

GDP by country

```{r}
#faltannn
#qc1,sd2t, qc20_multinomial
```
...
Data_final_Cntry <- Country_level |>
   select(country_name, prop_qc19_yes , prop_gndr_bin, prop_dis_wide,prop_lgb_yes, trans_legal_protection)
```
...
Data_final_Ind <- Individual_data |> 
  select(serialid,country_name,isocntry,binary_qc19,male, d11, Cat_age_four,Cat_age_six, Cat_age_seven,political_ideology,Religion_cat , sd1_7_factor, age_stopped_education, d60_ordinal,sd2_5,area_type,qc15_1_ordinal)
...
impute something? years of eductaion variable! age_stop_education
...
## Definition of yes
Data_final_Ind$Yes = factor(Data_final_Ind$binary_qc19==1) # less than 5 over 20
Data_final_Ind$binary_qc19 = NULL

prop.table(table(Data_final_Ind$Yes))
```

Here we split the data into testing and training with a 75% split.
...
Remember predictions are posterior probabilities:
